...
// command转换成text的缓存(提升性能)
const command2TextCache = new Array(1024);




// 使用缓存处理对象转数组以提升效率
const arrayCache = new Array(1024);
...
function toArray(values) {
    ...
    if (values)
    {
        let cache = arrayCache;
        let index = 0;
    
        for (let name in values)
        {
            cache[index++] = name;
            cache[index++] = values[name];
        }
...
module.exports = class Redis {
    ...
    constructor(options) {
        ...
        options.port = options.port > 0 ? options.port | 0 : 6379;
        options.timeout = options.timeout > 0 ? options.timeout | 0 : 30000;
        options.pipeline = options.pipeline > 0 ? options.pipeline | 0 : 32;
        options.cluster = options.cluster && initCluster(options.cluster);

        // 初始化指令队列(一次性发送至服务的指令数量)
        this.queue = Queue(this, options.pipeline);

        // 命令回复解析器
        this.parser = new Parser(this);

        // 解析结果响应后直接转发到消息队列的done处理
        this.parser.ondata = this.queue.done;
    ...
    connect() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.socketProxy.connect(resolve, reject);
        });
    }
    ...
    onconnected() {
        ...
        console.log('Redis connected to ' + this.options.host + ':' + this.options.port, new Date());
    }


    onerror(err) {
        
        console.error('fail to connect redis server', new Date());
        console.log(err);
    }
    ...
    onparseerror(err) {
        ...
        console.error('redis parse error', new Date());
        console.error(err);
    }
    ...
    command(key, ...args) {
        ...
        for (let i = 0; i < length; i++)
        {
            ...
            if (value == null)
            {
                return new Promise((_, reject) => {
                    ...
                    reject(new Error('send command error: value can not be null!'));
                });
            }
            ...
            switch (typeof value)
            {
                case 'boolean':
                    cache[index++] = '$1';
                    cache[index++] = value ? 1 : 0;
                    break;

                case 'number':
                    value = '' + value; // 小数按字符串处理
                    cache[index++] = '$' + fn(value);
                    cache[index++] = value;
                    break;

                case 'string':
                    cache[index++] = '$' + (value ? fn(value) : 0);
                    cache[index++] = value;
                    break;
        ...
        cache[index++] = '';
        cache = cache.slice(0, index).join('\r\n');

        return this.queue.push(cache);
    }
    ...
    sort(key, BY, pattern, LIMIT, offset, count/*, [GET, pattern]* */, ASC_OR_DESC, ALPHA, STORE, destination) {

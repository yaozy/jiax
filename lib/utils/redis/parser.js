...
const bufferToUtf8 = require('../buffer').utf8;
...
class BaseTask {
    ...
    resume(buffer, start) {
        ...
        if (index >= 0)
        {
            ...
            // 合并buffer缓存并转成字符串
            cache = bufferToUtf8(cache, this.cacheIndex);
...
class ArrayTask {
    ...
    readArray(buffer, start, array, unread) {
        ...
        while (start < length)
        {
            ...
            {
                throw new Error('not support type ' + (String.fromCharCode(buffer[start]) || buffer[start]));
            }
